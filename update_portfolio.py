...
def generate_scenario(name, price, prev_price, high_30, low_30, rsi, macd_signal):
    ...
    rate_day = round(((price - prev_price) / prev_price) * 100, 2)
...
def main():
    ...
    for item in holdings:
        ...
        scenario_info = generate_scenario(name, curr_price, prev_price, max(highs[-30:]), min(lows[-30:]), rsi, macd.iloc[-1] > signal.iloc[-1])

        signals = []
        if rsi <= 35: signals.append("매수신호: RSI 과매도")
        elif rsi >= 70: signals.append("매도신호: RSI 과열")
        if macd.iloc[-1] > signal.iloc[-1] and macd.iloc[-2] <= signal.iloc[-2]: signals.append("매수신호: MACD 골든크로스")
    ...
    market_brief = {
        ...
        "summary": "반도체 및 AI 세터를 중심으로 수급 유입이 지속되고 있으며, 시장 상황에 맞춘 손절가 준수 및 분할 대응이 유효합니다.",
